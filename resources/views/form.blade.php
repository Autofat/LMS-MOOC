...
<head>
    ...
    <title>Form Buat Soal - KemenLH/BPLH E-Learning Platform</title>
...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen py-8 subtle-pattern">
    <div class="max-w-4xl mx-auto px-4 py-8 subtle-pattern">
        <!-- Back Button - Close to Container -->
        <div class="mb-4">
            @php
                $materialId = request('material_id') ?? ($autoFillData['material_id'] ?? null);
                $backUrl = $materialId ? route('materials.show', $materialId) : route('questions.manage');
                $backText = $materialId ? 'Kembali ke Materi' : 'Kembali ke Kelola Soal';
            @endphp
            <a href="{{ $backUrl }}"
                class="text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center space-x-2 w-fit"
        ...
        <div class="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-8 border"
            ...
            <div class="text-center mb-8">
                <div class="professional-gradient p-6 rounded-2xl mb-6">
                    ...
                    <p class="text-blue-100">Buat soal pilihan ganda dengan manual</p>
                </div>
            </div>
            ...
            <form action="{{ route('questions.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="explanation" name="explanation" rows="4"
                        class="w-full px-4 py-3 rounded-xl shadow-sm focus:outline-none focus:ring-2 transition-all border-2"
                        style="border-color: rgba(28,88,113,0.2); background: linear-gradient(135deg, rgba(251,191,36,0.05) 0%, rgba(245,158,11,0.05) 100%);"
