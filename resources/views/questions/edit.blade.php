...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen subtle-pattern">
    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div class="bg-white/95 backdrop-blur-sm shadow-xl rounded-2xl p-8 border"
            ...
            <form method="POST" action="{{ route('questions.update', $question->id) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="option_a" class="block text-sm font-semibold mb-3 flex items-center space-x-2"
                            style="color: rgba(28,88,113,0.9);">
                            <span class="text-white px-2 py-1 rounded-lg text-xs font-bold"
                                style="background-color: rgba(28,88,113,1);">A</span>
                    ...
                    <div>
                        <label for="option_b" class="block text-sm font-semibold mb-3 flex items-center space-x-2"
                            style="color: rgba(28,88,113,0.9);">
                            <span class="text-white px-2 py-1 rounded-lg text-xs font-bold"
                                style="background-color: rgba(28,88,113,1);">B</span>
                    ...
                    <div>
                        <label for="option_c" class="block text-sm font-semibold mb-3 flex items-center space-x-2"
                            style="color: rgba(28,88,113,0.9);">
                            <span class="text-white px-2 py-1 rounded-lg text-xs font-bold"
                                style="background-color: rgba(28,88,113,1);">C</span>
                    ...
                    <div>
                        <label for="option_d" class="block text-sm font-semibold mb-3 flex items-center space-x-2"
                            style="color: rgba(28,88,113,0.9);">
                            <span class="text-white px-2 py-1 rounded-lg text-xs font-bold"
                                style="background-color: rgba(28,88,113,1);">D</span>
                    ...
                    <div>
                        <label for="option_e" class="block text-sm font-semibold mb-3 flex items-center space-x-2"
                            style="color: rgba(28,88,113,0.9);">
                            <span class="text-white px-2 py-1 rounded-lg text-xs font-bold"
                                style="background-color: rgba(28,88,113,1);">E</span>
                ...
                <div>
                    ...
                    <select id="answer" name="answer" required
                        ...
                        <option value="E" {{ old('answer', $question->answer) == 'E' ? 'selected' : '' }}>E
                        </option>
                    </select>
                    @error('answer')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="difficulty" name="difficulty" required
                        ...
                        <option value="medium"
                            {{ old('difficulty', $question->difficulty) == 'medium' ? 'selected' : '' }}>Medium
                        </option>
                        <option value="hard"
                ...
                <div class="flex justify-between items-center pt-8 border-t"
                    ...
                    <div class="text-sm" style="color: rgba(28,88,113,0.6);">
                        ...
                        <p><i class="fas fa-calendar-plus mr-1"></i>Dibuat:
                            {{ $question->created_at->format('d M Y H:i') }}</p>
                        @if ($question->updated_at != $question->created_at)
                            <p><i class="fas fa-edit mr-1"></i>Terakhir diupdate:
                                {{ $question->updated_at->format('d M Y H:i') }}</p>
                        @endif
