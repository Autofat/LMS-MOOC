...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen subtle-pattern">
                        ...
                        <select id="category" name="category" required
                            ...
                            @foreach($existingCategories as $cat)
                                <option value="{{ $cat->name }}" 
                                        {{ old('category', $material->category ?? ($material->subCategory ? $material->subCategory->category->name : '')) == $cat->name ? 'selected' : '' }}
    ...
    <script>
        ...
        // Sub category filtering based on selected category
        document.addEventListener('DOMContentLoaded', function() {
            // Get URL parameters
            const urlParams = new URLSearchParams(window.location.search);
            const fromSubcategory = urlParams.get('from') === 'subcategory';
            ...
            const materialSubCategoryId = @json($material->sub_category_id);
            const materialCategory = @json($material->category ?? ($material->subCategory ? $material->subCategory->category->name : ''));
                ...
                if (category && category.sub_categories && category.sub_categories.length > 0) {
                    ...
                    if (activeSubCategories.length > 0) {
                        ...
                        activeSubCategories.forEach(subCat => {
                            ...
                            // Auto-select if this matches the current material's subcategory or URL parameter
                            if ((materialSubCategoryId && materialSubCategoryId == subCat.id) || 
                                (subcategoryId && subcategoryId == subCat.id)) {
            ...
            if (subcategoryId) {
                ...
                const parentCategory = categoriesData.find(cat => 
                    cat.sub_categories && cat.sub_categories.some(subCat => subCat.id == subcategoryId)
                );
                ...
                // Initialize subcategories on page load for existing material
                updateSubCategories();
            }
        });
    </script>
