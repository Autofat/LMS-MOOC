...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen subtle-pattern">
    ...
    <div class="container mx-auto px-4 pb-8">
        @if ($materials->count() > 0 || $categories->count() > 0)
            <!-- Statistics Cards -->
                ...
                <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-6 border" style="border-color: rgba(28,88,113,0.2);">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold" style="color: rgba(28,88,113,1);">{{ $materials->total() }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-6 border" style="border-color: rgba(28,88,113,0.2);">
                            ...
                            @foreach($categories as $category)
                                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200 hover:shadow-lg transition-all duration-300">
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        @if($category->questions_count > 0)
                                            <a href="{{ route('materials.download.category.excel', ['category' => $category->name]) }}"
                                               ...
                                               title="Download semua {{ $category->questions_count }} soal dari kategori {{ $category->name }}">
                                                <i class="fas fa-file-excel mr-1"></i>Download Soal ({{ $category->questions_count }})
                                            </a>
                                        @else
                                        ...
                                        <button class="edit-category-btn bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white py-2 px-3 rounded-lg text-sm transition-all duration-300 transform hover:scale-105"
                                        ...
                                        <form method="POST" action="{{ route('categories.destroy', ['id' => $category->id]) }}"
                                            onsubmit="event.preventDefault(); showDeleteConfirmation(this, 'Yakin ingin menghapus kategori {{ $category->name }}? Semua materi dan soal dalam kategori ini akan dihapus.'); return false;" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="text-gray-500 mb-4">Buat kategori pertama untuk mengelompokkan materi pembelajaran Anda</p>
                            <button onclick="showAddCategoryModal()" 
            ...
            <!-- Materials Grid (Full List) -->
            <div id="materials-full-list" class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-6 border mb-8" style="border-color: rgba(28,88,113,0.2);">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="text-2xl font-bold mb-2" style="color: rgba(28,88,113,1);">
                        ...
                        <p class="text-sm text-gray-600">
                            <i class="fas fa-info-circle mr-1" style="color: rgba(28,88,113,0.7);"></i> 
                            Kelola dan generate soal otomatis dari materi pembelajaran menggunakan AI.
                        </p>
                    </div>
                    <div>
                        <a href="{{ route('materials.create') }}" 
                            ...
                            <div class="flex space-x-2">
                                ...
                                @if($material->questions_count > 0)
                                    <a href="{{ route('materials.download.questions.excel', $material->id) }}"
                                        ...
                                        onmouseover="this.style.background='linear-gradient(135deg, rgba(49,139,179,1) 0%, rgba(56,156,201,1) 100%)'"
                                        onmouseout="this.style.background='linear-gradient(135deg, rgba(42,122,157,1) 0%, rgba(49,139,179,1) 100%)'"
                                        title="Download {{ $material->questions_count }} soal dalam format Excel">
                                        <i class="fas fa-file-excel mr-1"></i>Download Soal ({{ $material->questions_count }})
                                    </a>
                                @else
                                    <span class="flex-1 text-gray-500 text-center py-2 px-3 rounded-lg text-sm bg-gray-200"
        ...
        @endif
            </div> <!-- Penutup Materials Grid Section -->
        @else
            ...
            </div> <!-- Penutup Materials Grid Section untuk empty state -->
        @endif
    </div>
            ...
            <form id="addCategoryForm" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           placeholder="Contoh: Pengelolaan Limbah, AMDAL, dll..."
                           maxlength="100">
                ...
                if (data.success) {
                    ...
                    successToast.innerHTML = `
                        <div class="flex items-center">
                            ...
                            <button onclick="hideMaterialsToast('categorySuccessToast')" class="ml-4 text-white hover:text-gray-200">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                    `;
                    ...
                    setTimeout(() => {
                        hideMaterialsToast('categorySuccessToast');
                    }, 5000);
                ...
                if (data.success) {
                    ...
                    successToast.innerHTML = `
                        <div class="flex items-center">
                            <i class="fas fa-check-circle text-2xl mr-4"></i>
                            <p class="text-base font-medium">${data.message}</p>
                            <button onclick="hideMaterialsToast('categoryUpdateSuccessToast')" class="ml-4 text-white hover:text-gray-200">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                    `;
                    ...
                    // Auto hide toast after 5 seconds
                    setTimeout(() => {
                        hideMaterialsToast('categoryUpdateSuccessToast');
                    }, 5000);
